...
import { saveHillStation } from '@/app/admin/hill-stations/actions'
import ImageUpload from './ImageUpload'
import Link from 'next/link'
import { useRouter } from 'next/navigation'

export default function HillStationForm({ initialData, id }: { initialData?: any, id: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await saveHillStation(formData)
      if (result?.error) {
        alert(result.error)
        setLoading(false)
      } else {
        router.push('/admin/hill-stations')
      }
    } catch (err: any) {
      alert(err.message)
      setLoading(false)
    ...
    <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl bg-white p-8 rounded-xl shadow-sm border border-gray-100">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4 md:col-span-2">
          <h3 className="text-lg font-bold border-b pb-2">Hill Station Details</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Category (e.g. Hill Station, Escape)</label>
            ...
            <div className="md:col-span-2">
              ...
              <input type="text" name="custom_link" defaultValue={initialData?.custom_link} placeholder="e.g. /packages/araku-valley or https://google.com" className="w-full p-2.5 border rounded-lg" />
            </div>
            <div className="md:col-span-2">
        ...
        <div className="md:col-span-2 pt-4 border-t flex items-center justify-between">
          ...
          <div className="flex gap-4">
            <Link href="/admin/hill-stations" className="px-6 py-2.5 border rounded-lg font-medium hover:bg-gray-50">
              Cancel
            </Link>
            <button type="submit" disabled={loading} className="px-6 py-2.5 bg-charcoal text-white rounded-lg font-medium hover:bg-charcoal/90 disabled:opacity-50">
              {loading ? 'Saving...' : 'Save Hill Station'}
            </button>
          </div>
        </div>
