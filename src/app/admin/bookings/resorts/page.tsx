...
import { updateResortBookingStatus } from '@/app/actions/booking'
...
export default async function ResortBookingsPage(props: { searchParams?: Promise<{ [key: string]: string | undefined }> | { [key: string]: string | undefined } }) {
  const supabase = await createClient()
  const resolvedParams = await props.searchParams
  const statusFilter = resolvedParams?.status || 'all'

  let query = supabase.from('resort_bookings').select('*').order('created_at', { ascending: false })
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="p-4 font-semibold text-sm text-gray-600">Resort Details</th>
              <th className="p-4 font-semibold text-sm text-gray-600">Stay Dates</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {!bookings?.length ? (
              <tr><td colSpan={6} className="p-8 text-center text-gray-500">No resort bookings found</td></tr>
            ) : (
              bookings.map((booking) => (
                <tr key={booking.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4 align-top">
                    <p className="font-bold text-gray-900 text-sm">{booking.resort_name}</p>
                    <p className="text-sm text-coral font-medium mt-1">{booking.room_type}</p>
                  ...
                  <td className="p-4 align-top text-right">
                    <StatusActions id={booking.id} status={booking.status} updateAction={updateResortBookingStatus} />
                  </td>
                </tr>
              ))
            )}
